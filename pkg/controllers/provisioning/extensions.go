...
func (e *InstanceTypeExtender) init(ctx context.Context, kubeClient client.Client) {
	extensionList := &v1beta1.InstanceTypeExtensionList{}
	err := kubeClient.List(ctx, extensionList)
	if err != nil {
		logging.FromContext(ctx).Errorf("failed listing extensions, err: %s", err)

		return
	}

	for _, instanceTypeExtension := range extensionList.Items {
		for _, v1betaextension := range instanceTypeExtension.Spec.Extensions {
			regex, err := regexp.Compile(v1betaextension.InstanceNameRegex)
			if err != nil {
			...
			e.extensions = append(e.extensions, &extension{
				regex:             regex,
				requiredNPLabels:  v1betaextension.RequiredNodePoolLabels,
				extendedResources: v1betaextension.ExtendedResources,
			})
		}
	}
}

func newInstanceTypeExtender(ctx context.Context, kubeclient client.Client) *InstanceTypeExtender {
	e := &InstanceTypeExtender{
		solvedMatches: map[string]map[string]*extension{},
		extensions:    []*extension{},
	}

	e.init(ctx, kubeclient)

	return e
}

func GetInstanceTypeExtender(ctx context.Context, kubeclient client.Client) *InstanceTypeExtender {
	extOnce.Do(func() {
		extender = newInstanceTypeExtender(ctx, kubeclient)
	})
